...
class Statistical_Validation(DiTToLayerBase):
    ...
    def apply(cls, stack, model, *args, **kwargs):
        ...
        partitioned_command = 'module purge; module load R; module load gcc/7.2.0; module load openmpi-gcc/2.1.2-7.2.0; cd {smartdsR}; Rscript {rscript} {input} {output} {name}'.format(smartdsR=rscript_folder, rscript=partitioned_file, input=input_folder, output=output_folder, name=output_name)
        non_partitioned_command = 'module purge; module load R; module load gcc/7.2.0; module load openmpi-gcc/2.1.2-7.2.0; cd {smartdsR}; Rscript {rscript} {input} {output} {name}'.format(smartdsR=rscript_folder, rscript=non_partitioned_file, input=input_folder, output=output_folder, name=output_name)
