...
def create_rnm_to_opendss_stack_scenarios(dataset_dir, region, solar, batteries):
    ...
    split.kwargs['excel_output'] = os.path.join('.', 'results_v2', region, 'timeseries_solar_'+solar+'_battery_'+batteries,'opendss', 'metrics.csv')
    ...
    stack.save(os.path.join(stack_library_dir,'rnm_to_opendss_stack_timeseries_'+region+'solar_'+solar+'_batteries_'+batteries+'.json'))
...
#create_rnm_to_opendss_stack(os.path.join('..','..','dataset3', 'MixedHumid'), 'industrial')
    ...
    from layerstack.stack import Stack
    s = Stack.load('../stack_library/rnm_to_opendss_stack_timeseries_'+region+'solar_'+solar+'_batteries_'+batteries+'.json')
    if not os.path.isdir(os.path.join('.','results_v2',region, 'timeseries_solar_'+solar+'_battery_'+batteries,'opendss')):
        os.makedirs(os.path.join('.','results_v2',region, 'timeseries_solar_'+solar+'_battery_'+batteries,'opendss'))
    s.run_dir = 'run_dir'
    s.run()
