...
def create_rnm_to_cyme_stack_pv(dataset_dir, region, pct_pv=15):
    ...
    add_timeseries.kwargs['output_folder'] = os.path.join('.','results',region,'timeseries_{pct}_pv'.format(pct=pct_pv),'cyme')
    ...
    split.kwargs['excel_output'] = os.path.join('.', 'results', region, 'timeseries_{pct}_pv'.format(pct=pct_pv),'cyme', 'metrics.csv')
    split.kwargs['json_output'] = os.path.join('.', 'results', region,  'timeseries_{pct}_pv'.format(pct=pct_pv),'cyme', 'metrics.json')
    ...
    #Write to CYME
    final = stack[19]
    final.args[0] = os.path.join('.','results',region,'timeseries_{pct}_pv'.format(pct=pct_pv),'cyme')

    #Write Tags
    tags = stack[20]
    tags.kwargs['output_folder'] = os.path.join('.','results',region,'timeseries_{pct}_pv'.format(pct=pct_pv),'cyme')
    tags.kwargs['tag_file'] = os.path.join(dataset_dir,region,'Auxiliary','FeederStats.txt')

    stack.save(os.path.join(stack_library_dir,'rnm_to_cyme_stack_timeseries_pv_'+region+'_'+str(pct_pv)+'_pct.json'))
...
#create_rnm_to_cyme_stack(os.path.join('..','..','dataset3', 'MixedHumid'), 'industrial')
    ...
    if not os.path.isdir(os.path.join('.','results',region,'timeseries_{pct}_pv'.format(pct=percent),'cyme')):
        os.makedirs(os.path.join('.','results',region,'timeseries_{pct}_pv'.format(pct=percent),'cyme'))
    create_rnm_to_cyme_stack_pv(os.path.join('..','..','{dset}_{date}'.format(dset=dataset,date = dataset_map[dataset])), region,percent)
    from layerstack.stack import Stack
    s = Stack.load('../stack_library/rnm_to_cyme_stack_timeseries_pv_'+region+'_'+str(percent)+'_pct.json')
    s.run_dir = 'run_dir'
    s.run()
