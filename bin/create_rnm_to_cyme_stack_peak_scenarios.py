...
def create_rnm_to_cyme_stack_scenarios(dataset_dir, region, solar, batteries):
    ...
    split.kwargs['excel_output'] = os.path.join('.', 'results_v2', region, 'peak_solar_'+solar+'_battery_'+batteries,'cyme', 'metrics.csv')
    split.kwargs['json_output'] = os.path.join('.', 'results_v2', region, 'peak_solar_'+solar+'_battery_'+batteries, 'cyme','metrics.json')
    ...
    transformer_metrics.kwargs['output_folder'] = os.path.join('.','results_v2',region, 'peak_solar_'+solar+'_battery_'+batteries,'cyme')
   ...
   #Add additional regulators
    ...
    #Write to CYME
    final = stack[27]
    final.args[0] = os.path.join('.','results_v2',region, 'peak_solar_'+solar+'_battery_'+batteries,'cyme')

    #Dump to Ditto json
    final_json = stack[28]
    final_json.kwargs['base_dir'] = os.path.join('.','results_v2',region, 'peak_solar_'+solar+'_battery_'+batteries,'json_cyme')

    #Write Tags 
    tags = stack[29]
    tags.kwargs['output_folder'] = os.path.join('.','results_v2',region, 'peak_solar_'+solar+'_battery_'+batteries,'cyme')
    tags.kwargs['tag_file'] = os.path.join(dataset_dir,region,'Auxiliary','FeederStats.txt')

    #Write validation
    validation = stack[30]
    validation.kwargs['output_folder'] = os.path.join('.','results_v2',region, 'peak_solar_'+solar+'_battery_'+batteries,'cyme')
    validation.kwargs['input_folder'] = os.path.join('.','results_v2',region, 'peak_solar_'+solar+'_battery_'+batteries,'cyme')
    validation.kwargs['rscript_folder'] = os.path.join('..','..','smartdsR-analysis-lite')
    validation.kwargs['output_name'] = region

    stack.save(os.path.join(stack_library_dir,'rnm_to_cyme_stack_peak_'+region+'solar_'+solar+'_batteries_'+batteries+'.json'))
...
#create_rnm_to_cyme_stack(os.path.join('..','..','dataset3', 'MixedHumid'), 'industrial')
    ...
    from layerstack.stack import Stack
    s = Stack.load('../stack_library/rnm_to_cyme_stack_peak_'+region+'solar_'+solar+'_batteries_'+batteries+'.json')
    if not os.path.isdir(os.path.join('.','results_v2',region, 'peak_solar_'+solar+'_battery_'+batteries,'cyme')):
        os.makedirs(os.path.join('.','results_v2',region, 'peak_solar_'+solar+'_battery_'+batteries,'cyme'))
    s.run_dir = 'run_dir'
    s.run()
